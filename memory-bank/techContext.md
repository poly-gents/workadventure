...
- **Location**: `back/src/Model/GameRoom.ts`
- **Role**: `GameRoom` is now responsible for maintaining the complete state of players and map objects.
- **Key Modifications**:
    1.  **State Properties**: Added `playersState` and `mapObjectsState` to hold live data.
    2.  **Lifecycle Integration**: The `join`, `leave`, and `handleAgentRegistration` methods now update the `playersState`. The `mapObjectsState` is populated when the room is created.
    3.  **State Persistence**: The state is serialized to JSON and saved as a room variable whenever it changes, making it available to external systems.
- **Associated Files**:
  - `back/src/Model/State.ts`: Defines the TypeScript interfaces (`PlayerStateInfo`, `MapState`, etc.) for the new state variables. This is the canonical schema for the state.
  - `back/src/Model/Group.ts`: Modified to include public getters (`id`, `members`) to allow safe access to group properties.
  - `back/src/Services/SocketManager.ts`: Updated to use the new public getters on the `Group` class, resolving runtime errors.

### Accessing Room State via Room API
The centralized state is exposed as room variables. To read the state, use the `roomApiClient`.

-   **Player State**:
    -   **Variable Name**: `"players_state"`
    -   **Format**: A JSON string representing a Map of `[playerId, PlayerStateInfo]`.
    -   **Client-side Reconstruction**: `new Map(JSON.parse(variableValue))`
-   **Map Objects State**:
    -   **Variable Name**: `"map_objects_state"`
    -   **Format**: A JSON string of a complex object containing nested Maps for layers and objects. Requires a custom reconstruction logic on the client (see implementation for details).
